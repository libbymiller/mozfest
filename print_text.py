...
class ThermalPrinter(object):
    ...
    def __init__(self, heatTime=80, heatInterval=2, heatingDots=7, serialport=SERIALPORT):
        ...
        self.printer.write(chr(heatInterval)) # Heat interval (384 uS = slower, but darker) default = 250
...
#make line height large 
    ...
    def font_b_on(self):
        ...
        self.printer.write(chr(33))
        self.printer.write(chr(1))
#make line height small
        self.printer.write(self._ESC)
        self.printer.write(chr(51))
...
if __name__ == '__main__':
    ...
    if not len(sys.argv) > 4:
        sys.exit("Usage ./print_text.py image id power time [friends company aps]")

    print "Printing via port %s" % serialport
...
# get ascii image
    ff = subprocess.check_output("jp2a --width=42 "+image, shell=True)
    ff_arr = ff.split("\n")
...
#    p.font_b_on()
    ...
    if(len(sys.argv)>6 and sys.argv[6]):
      company = sys.argv[6]
      p.print_text("Manufacturer: "+company)
      p.linefeed()
    p.print_text("Signal strength: "+power)
    p.linefeed()
    if(len(sys.argv)>7 and sys.argv[7]):
      aps = sys.argv[7]
      p.print_text("Your wifi networks: "+aps)
      p.linefeed()
      p.linefeed()
    if(len(sys.argv)>5 and sys.argv[5]):
      friends = sys.argv[5]
      frs = friends.split(",")
      p.print_text("Associated devices: ")
      for fr in frs:
        p.print_text(fr)
        p.linefeed()

    p.linefeed()
    p.print_text("Seen at "+t)
    p.linefeed()
